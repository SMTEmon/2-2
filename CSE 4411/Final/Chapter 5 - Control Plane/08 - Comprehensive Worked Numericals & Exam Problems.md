...
tags:
  ...
  - control-plane
aliases:
  - Control Plane Practice Problems
  - Chapter 5 Numericals
---

# 08 - Comprehensive Worked Numericals & Exam Problems (Control Plane)

> [!abstract] Exam Objective
> This document contains full, rigorous mathematical solutions for **Dijkstra's Link-State tables**, **Distance-Vector Bellman-Ford matrices**, and **BGP Route Selection** problems.

---

## 🧮 Problem Set 1: Complete Dijkstra's Algorithm Execution
...
#### Iteration 1 (After First Vector Exchange)
- **Node $x$ computes $D_x(z)$ using Bellman-Ford:**
  $$D_x(z) = \min \{ c(x,y) + D_y(z), c(x,z) + D_z(z) \} = \min \{ 2 + 1, 7 + 0 \} = \min \{ 3, 7 \} = \mathbf{3} \quad (\text{via } y)$$
- **Node $z$ computes $D_z(x)$ using Bellman-Ford:**
  $$D_z(x) = \min \{ c(z,y) + D_y(x), c(z,x) + D_x(x) \} = \min \{ 1 + 2, 7 + 0 \} = \min \{ 3, 7 \} = \mathbf{3} \quad (\text{via } y)$$
- **Node $y$** sees no changes to its shortest paths.

#### Iteration 2 (Final Stable State)
...
**Question:** Which route will Router $R1$ select? Provide step-by-step elimination.

---

### Step-by-Step Selection Process

1. **Step 1: Check `LOCAL_PREF` (Highest wins):**
   - Route A: `LOCAL_PREF = 100`
   - Route B: `LOCAL_PREF = 100`
   - **Route C: `LOCAL_PREF = 150`**
2. **Decision:** **Route C is immediately selected!**
   - *Even though Route C has a longer `AS-PATH` (3 hops vs 2 hops) and higher internal OSPF cost (25 vs 10), `LOCAL_PREF` is the highest-priority criterion in BGP policy.*

---
#### Navigation
← Previous: [[07 - Book Extras & Professor Traps]] | Next: [[00 - Index]] (Chapter 6) →
