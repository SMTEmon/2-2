\documentclass[12pt,landscape,a4paper]{extarticle}
...
\lstset{
  ...
  breaklines=true,
  breakindent=1em,
  postbreak=\mbox{\textcolor{cx}{$\hookrightarrow$}\space},
  tabsize=2,
...
\algo{Counting Sort (negative-safe)}{$O(n+m)$ time \& space, stable}
\note{$m$ = value range $=\max-\min$; offset shifts negatives to $\ge 0$}
\begin{lstlisting}
vector<int> countingSort(vector<int>& A){
  int n=A.size();
  int mn=*min_element(A.begin(),A.end());
  int mx=*max_element(A.begin(),A.end());
  int range=mx-mn+1, off=-mn;         // handle negatives
  vector<int> cnt(range,0), res(n);
  for(int x:A) cnt[x+off]++;          // frequency
  for(int i=1;i<range;i++) cnt[i]+=cnt[i-1]; // prefix
...
\algo{Dijkstra --- Path Reconstruction}{track parents, walk back}
\begin{lstlisting}
// par[v] = predecessor of v on shortest path
vector<int> shortestPath(
    vector<vector<pair<int,int>>>& adj,
    int V, int src, int dst){
  ...
  while(!pq.empty()){
    ...
    for(auto [v,w]:adj[u])
      if(!vis[v] && d+w<dist[v]){
        dist[v]=d+w; par[v]=u;      // remember parent
        pq.push({dist[v],v}); }
  }
  vector<int> path;                 // rebuild dst -> src
  if(dist[dst]==INT_MAX) return path;  // unreachable
  for(int u=dst; u!=-1; u=par[u]) path.push_back(u);
...
vector<int> bellmanFord(int V,
  ...
  for(int i=0;i<V;i++){         // V-1 relax + 1 check
    ...
    if(!changed) break;         // converged early -> speedup
  }
  return dist;
}
\end{lstlisting}
